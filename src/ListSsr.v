...
Lemma perm_cat_cons (T : eqType) (x : T) : forall (s1 s2 : seq T),
...
Lemma subseq_in_cons : forall (a : eqType) x y (xs ys : list a),
...
Lemma in_subseq_sing : forall {E : eqType} (s : list E) v (y : E) ys,
...
Ltac sortedness :=
  ...
  repeat
    match goal with
    | [ H : StronglySorted ?R (?XS ++ ?YS) |- _ ] =>
      apply StronglySorted_inv_app in H; destruct H
    | [ |- StronglySorted ?R (?XS ++ ?YS)%SEQ ] =>
      apply StronglySorted_cat
    | [ H : StronglySorted ?R (?X :: ?XS) |- _ ] =>
      apply StronglySorted_inv in H; destruct H
    | [ |- StronglySorted ?R (?X :: ?XS) ] =>
      constructor
    | [ H : List.Forall ?R (?XS ++ ?YS) |- _ ] =>
      apply Forall_append in H; destruct H
    | [ |- List.Forall ?R (?XS ++ ?YS) ] =>
      apply Forall_append; split
    | [ H : List.Forall ?P (?X :: ?XS) |- _ ] =>
      apply List.Forall_inv in H; destruct H
    | [ |- List.Forall ?P (?X :: ?XS) ] =>
      constructor
    | [ H : is_true (all ?P ?L) |- _ ] => move/Forall_all in H
    | [ |- is_true (all ?P ?L) ] => apply/Forall_all
    | [ H : List.Forall ?P ?XS |- List.Forall ?Q ?XS ] =>
      eapply (@List.Forall_impl _ P Q _ XS H)
    end;
  intuition;
  eauto.
