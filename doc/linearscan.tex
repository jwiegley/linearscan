...
% \DefineVerbatimEnvironment{MyCoqExample}{Verbatim}{frame=lines,framerule=1pt,fontsize=\small,samepage=true}
% \DefineVerbatimEnvironment{MyCoqExampleStar}{Verbatim}{frame=lines,framerule=1pt,fontsize=\small,samepage=true}
% \DefineVerbatimEnvironment{MyCoqUneval}{Verbatim}{frame=lines,framerule=1pt,fontsize=\small,samepage=true}
...
% \fixme{Rewrite section overview}In Section~XXX, we give a
...
%   in each new context.\gts{is this subsumed by the following item?}
...
% \gts{is ``existential data type'' the same as ``dependent data type''? If so,
%      let's just use ``dependent'' throughout.}
that pairs the
  qualified value with the function's result: an example would be
  \verb@{ l : list a | size l == 4 }@.

% , or specialized by a data  constructor as:
...
% \gts{English makes sense, but can't map it to code. Need to nknow
%   which of SState, SSMorphHasLen, SSMorphSt correspond to the
%   properties you mention. Also, what's ``option PhysReg'' doing?}
