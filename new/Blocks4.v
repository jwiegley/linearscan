...
Record Block : Type := {
    ...
    regRequired : bool;
    refCount    : nat;
    references  : refCount.-tuple SomeVar
}.
...
Record boundedRangeVec (pos : nat) := {
  vars : maxVirtReg.-tuple (boundedTriple pos);
  regs : maxReg.-tuple (boundedTriple pos)
}.

Definition transportVecBounds (pos m : nat) `(Hlt : pos < n) :
  m.-tuple (boundedTriple n) -> m.-tuple (boundedTriple pos).
Proof.
...
Definition processBlocks (blocks : NonEmpty Block) :
  maxVirtReg.-tuple (option RangeSig) * maxReg.-tuple (option RangeSig) :=
